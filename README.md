...
The Explore button click triggers a handler in app.js.
JavaScript reads the selected date from the date picker.
fetchFromApi() searches the image-article collection for that day's post.
The matching post is normalised into a simple object: title, date, explanation, media_type, url, hdurl.
displayAPOD() updates the page — image source, title, formatted date, explanation text — and removes the hidden class from the result container.
...
Today's picture loads automatically.
Pick a different date and click Explore to see that day's image.
